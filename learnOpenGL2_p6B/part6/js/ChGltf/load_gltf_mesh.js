import { vs_materials, fs_materials } from "../../js/ChGltf/shaders/index.js";
import { vec3, mat4 } from "../../../math/glmatrix/index.js";
import { Shader } from "../../js/common/shader.js";
import { PbrShader } from "../../js/common/PbrShader.js";
...
function resourcesLoaded(res) {
    ...
    let meshes = model.getMeshes(true);
    if (meshes.length > 0)
...
function createGlDrawable(model, mesh) {
    ...
    for (let j = 0, jlen = mesh.vertexObjects.length; j < jlen; j++) {
        ...
        let material = vo.material;
        let shader = new PbrShader(gl);
        if (material) {
            if (material.pbrMetallicRoughness.baseColorTexture) {
                shader.defineMacro('HAS_BASECOLORMAP');
            }
            if (material.pbrMetallicRoughness.metallicRoughnessTexture) {
                shader.defineMacro('HAS_METALROUGHNESSMAP');
            }
            if (material.normalTexture) {
                shader.defineMacro('HAS_NORMALMAP');
            }
            if (material.occlusionTexture) {
                shader.defineMacro('HAS_OCCLUSIONMAP');
            }
            if (material.emissiveTexture) {
                shader.defineMacro('HAS_EMISSIVEMAP');
            }
        }
        shader.compile();
        glMesh.shaders.push(shader);
