...
import { Glyph } from "../../js/geometry/Glyph.js";
const sizeFloat = 4;
...
let main = function () {
    ...
    fetch("../../fonts/AlexBrush/Alex Brush_Regular.json")
...
function afterLoad(jsonFont) {
    ...
    for (let i = 0, il = name.length; i < il; i++) {
        ...
        if (!jsonGlyph) {
            startTranslation += fontResolution;
            continue;
        }
        console.log(jsonGlyph.o);
        let glyph = new Glyph(jsonGlyph, true);
...
function render() {
    ...
    lightColor[1] = 1.0;
    lightColor[2] = 1.0;
    let diffuseColor = componentProduct3(lightColor, vec3.fromValues(0.8, 0.8, 0.8));
    ...
    for (let i = 0; i < glyphVAOS.length; i++) {
        ...
        gl.drawElements(gl.TRIANGLES, glyphObjects[i].indices.length, gl.UNSIGNED_SHORT, 0);
    }
    requestAnimationFrame(render);
}
